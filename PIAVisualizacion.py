...
class Ui_Dialog(QDialog):
    ...
    def Graficar(self):
        ...
        Campos=[i0,i2,i4,i6,i8]
        Valores=[i1,i3,i5,i7,i9]
        ...
        if self.cbMat.isChecked():
            if self.radioBarras.isChecked():
                ...
                plt.ylabel(item3)
                plt.title(item1)
                self.labelComentario.setText("Seleccionaste la Grafica de Barras con Matplotlib")
                plt.show()
            elif self.radioLineas.isChecked():
                df.groupby('Campos')['Valores'].sum().plot(kind='line', legend='Reverse', color="black")
                plt.xlabel(item2)
                plt.ylabel(item3)
                plt.title(item1)
                self.labelComentario.setText("Seleccionaste la Grafica de Lineas con Matplotlib")
                plt.show()
                ...
                plt.ylabel("")
                plt.title(item1)
                self.labelComentario.setText("Seleccionaste la Grafica de Pastel")
                plt.show()
            else:
        ...
        if self.cbSea.isChecked():
            if self.radioBarras.isChecked():
                ...
                plt.ylabel(item3)
                plt.title(item1)
                self.labelComentario.setText("Seleccionaste la Grafica de Barras con Seaborn.")
                plt.show()
            elif self.radioLineas.isChecked():
                ...
                plt.ylabel(item3)
                plt.title(item1)
                self.labelComentario.setText("Seleccionaste la Grafica de Lineas con Seaborn")
                plt.show()
            elif self.radioPastel.isChecked():
